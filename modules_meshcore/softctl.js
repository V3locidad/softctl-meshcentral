    ...
    runPs(detectPs, 30 * 1000, function (code, out, err) {
        ...
        download(msiUrl, msiPath, function (derr) {
            ...
            var msiPs = ''
                ...
                // Fix Error 1327 : patch temporaire des "User Shell Folders" pointant vers un lecteur inexistant
                + '$__shellBackups = @();'
                ...
                + '        if ($__v -match "^([A-Za-z]):") {'
                + '          $__d = $Matches[1] + ":\\";'
                + '          if (-not (Test-Path $__d)) {'
                + '            Write-Host ("SHELL_FIX:" + $__p.Name + "=" + $__v);'
                + '            $__shellBackups += [PSCustomObject]@{Key=$__k;Name=$__p.Name;Val=$__v};'
                + '            try { Set-ItemProperty -Path $__k -Name $__p.Name -Value ($env:SystemDrive + "\\Temp") -ErrorAction SilentlyContinue; } catch {}'
                + '          }'
                + '        }'
                + '      }'
                + '    }'
                ...
                + '} finally {'
                + '  foreach ($__b in $__shellBackups) {'
                + '    try { Set-ItemProperty -Path $__b.Key -Name $__b.Name -Value $__b.Val -ErrorAction SilentlyContinue; Write-Host ("SHELL_RESTORE:" + $__b.Name); } catch {}'
                + '  }'
                + '}';
